...
- **Rust port** `rpi/workplacesim/` — additive; not required for the
  ...
  framebuffer, Pi). Deploy via `rpi/workplacesim/deploy/install.sh
...
workplacesim/
  ...
  scripts/simulate.js
  rpi/workplacesim/               # Rust port for Raspberry Pi 1 / framebuffer
...
- Changes to shared behaviour (routing constants, classify rules, state
  ...
  desktop --no-default-features` from `rpi/workplacesim/` after any
