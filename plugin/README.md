...
- `WORKPLACESIM_URL` — base URL for the visualizer server. Defaults to
  `http://workplacesim.local:4317`, which resolves to the Raspberry Pi
  running the Rust visualizer via the in-binary mDNS responder set up by
  `rpi/workplacesim/deploy/install.sh`. Override with
  `export WORKPLACESIM_URL=http://127.0.0.1:4317` to point at a local
  Node server, or any other LAN address. The hook script silently drops
  events when the URL is unreachable so a missing visualizer never blocks
  a Claude session.
