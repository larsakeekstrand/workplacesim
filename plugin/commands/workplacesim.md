...
1. The server URL is `${WORKPLACESIM_URL:-http://workplacesim.local:4317}`. Use a 3-second curl timeout.
2. `GET /api/agents` — if it succeeds, parse the JSON and summarize:
   ...
   - For each, one line: `user@host  agent_type · description  → <room>` where `<room>` is determined by:
     ...
     - `desk` otherwise
3. If the request fails, say so plainly and suggest: start the Pi visualizer (or `npm start` from the workplacesim repo for the Node version), or set `WORKPLACESIM_URL` to point elsewhere.
